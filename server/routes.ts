...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/photos/user/:userId", async (req, res) => {
    const photos = await storage.getUserPhotos(parseInt(req.params.userId));
    res.json(photos);
  });
  ...
  app.post("/api/photos/:photoId/like", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    await storage.likePhoto(req.user.id, parseInt(req.params.photoId));
    res.sendStatus(200);
  });

  app.post("/api/photos/:photoId/unlike", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    await storage.unlikePhoto(req.user.id, parseInt(req.params.photoId));
    res.sendStatus(200);
  });
  ...
  app.post("/api/users/:userId/follow", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    await storage.followUser(req.user.id, parseInt(req.params.userId));
    res.sendStatus(200);
  });

  app.post("/api/users/:userId/unfollow", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    await storage.unfollowUser(req.user.id, parseInt(req.params.userId));
    res.sendStatus(200);
  });

  // Comment routes
  app.get("/api/photos/:photoId/comments", async (req, res) => {
    const comments = await storage.getPhotoComments(parseInt(req.params.photoId));
    res.json(comments);
  });
  ...
  app.get("/api/photos/:photoId/likes", async (req, res) => {
    const users = await storage.getLikedUsers(parseInt(req.params.photoId));
    res.json(users);
  });

  app.get("/api/users/:userId/followers", async (req, res) => {
    const followers = await storage.getFollowers(parseInt(req.params.userId));
    res.json(followers);
  });
